...
public abstract class AbstractTournamentTester
{
  ...
  public void testGetPairings() throws TournamentSignupException
  {
    ...
    Encounter e = result.values().toArray(new Encounter[0])[(tournament
            .getAmountOfTeams() / 2) - 1];
    TeamInterface t
    ...
    {
    })[tournament.getAmountOfTeams() / 2 - 1];
  ...
  public void testSimulateTournament()
  {
    ...
    for (int i = 0; i < 100; i++)
    {
      ...
      LOG.log(Level.INFO, "Amount of expected rounds: {0}",
              tournament.getMinimumAmountOfRounds());
          ...
          if (tournament.getActiveTeams().size() > 1)
          {
                    ...
                    : tournament.getPairings().entrySet())
            {
                      ...
                      && !player2.equals(TournamentInterface.BYE))
              {
                ...
                tournament.updateResults(encounter.getId(), player1,
